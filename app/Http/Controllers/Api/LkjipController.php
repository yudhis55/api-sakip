...
use App\Http\Resources\LkjipResource;
...
{
    /**
     * Display a listing of LKJIP documents.
     */
    public function index(Request $request)
    {
        $query = Lkjip::with(['opd', 'tahun.periode', 'user']);
        ...
        if ($request->boolean('all', false)) {
            $documents = $query->get();
            return LkjipResource::collection($documents);
        }
    ...
    /**
     * Display the specified LKJIP document.
     */
    public function show($id)
    {
        $document = Lkjip::with(['opd', 'tahun.periode', 'user'])->findOrFail($id);

        return new LkjipResource($document);
    }
}
